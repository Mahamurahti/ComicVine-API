...
function fetchComics(data) {
    fetch(proxyUrl + targetUrl + `search/?api_key=${config.API_KEY}&format=json&query=${data}&resources=volume`).
...
function printComicInformation(data) {
  ...
  for (let i = 0; i <= data.results.length; i++) {
    ...
    if(data.results[i].description == null && data.results[i].first_issue.name == "No name found"){
      section.innerHTML += `<section class="comicInfo">
            ...
            <p>For more details about this comic visit:</p>
    ...
    }else if(data.results[i].first_issue.name == "No name found"){
      section.innerHTML += `<section class="comicInfo">
            <img src="${data.results[i].image.medium_url}" alt="${data.results[i].name}">
            <h1>${data.results[i].name}</h1>
